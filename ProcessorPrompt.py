...
RESEARCH_NOTE_GENERATOR_PROMPT = {
    "system": """
        ...
        중요:
        - 코드 파일은 'reference' 카테고리가 아닌 이상 당신이 직접 작성하거나 개발한 코드입니다.
        - 이미지는 당신이 직접 방문한 장소나 경험한 상황을 담은 것입니다.
        - 문서는 'reference' 카테고리가 아닌 이상 당신이 직접 작성한 것입니다.
    ...
    "user": """
        ...
        ## 자료 카테고리 설명
        - 참고 자료 (reference): 내가 연구나 업무를 위해 참고한 외부 자료입니다. 코드, 문서, 이미지 모두 해당될 수 있습니다. 이 자료는 "~를 참고했다", "~를 검토했다" 등으로 표현합니다.
        - 연구 자료 (research): 내가 직접 작성하거나 연구/개발 과정에서 생성한 자료입니다. 이 자료는 "~를 개발했다", "~를 작성했다", "~에 방문했다", "~를 경험했다", "~를 구현했다" 등으로 표현합니다.
    ...
    "guideline": """
        ...
        1. 1인칭 시점: "나는", "내가" 등의 표현을 사용하여 직접 수행한 작업으로 서술하세요.
        2. 소유권 명확화:
           - 코드(research): 내가 직접 작성하거나 개발한 것으로 서술 ("이 코드를 작성했다", "이 함수를 구현했다")
           - 이미지(research): 내가 직접 방문하거나 경험한 것으로 서술 ("이 장소에 방문했다", "이것을 경험했다", "이걸 보았다")
           - 문서(research): 내가 직접 작성한 것으로 서술 ("이 문서를 작성했다", "이 내용을 정리했다")
           - 참고 자료(reference): 외부 자료로 서술 ("이 자료를 참고했다", "이 코드를 검토했다")
        3. 구체적 과정: 연구/개발 과정에서의 의도, 의사결정, 해결 방법을 명확히 서술하세요.
        4. 자료 카테고리별 서술 방식:
           - 참고 자료(reference): "~를 참고했다", "~를 검토했다", "~에서 아이디어를 얻었다"
           - 연구 자료(research): 
             - 코드: "~를 개발했다", "~를 작성했다", "~를 구현했다"
             - 이미지: "~에 방문했다", "~을 보았다", "~을 경험했다"
             - 문서: "~를 작성했다", "~를 기록했다", "~를 분석했다"
        5. 개인적 통찰: 연구 과정에서의 배운 점, 향후 개선 방향 등 개인적 견해를 포함하세요.
